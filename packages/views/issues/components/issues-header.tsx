...
import { getActiveFilterCount } from "../utils/filter";
...
export function IssueFilterButton({ scopedIssues }: { scopedIssues: Issue[] }) {
  const { t } = useT("issues");
      ...
      <Tooltip>
        <DropdownMenuTrigger
          render={
            <TooltipTrigger
              render={
                <Button variant="outline" size="icon-sm" className="relative text-muted-foreground" aria-label={t(($) => $.filters.tooltip)}>
